...
class taikhoan
{
	...
	function saveTK($cus)
	{
		...
		if ($row_Email != 0) {
			...
			echo "<script>alert('Đăng kí thành công , hãy đăng nhập.');window.location='login.php';
				</script>";
		}
	}
	function getTK($matk)
	...
	function lockTK($matk,$id)
	{
		$a = new DateBase;
		$con = $a->connnection();
		$sql = "update taikhoan set KichHoat='".$id."' where MaTaiKhoan='" . $matk . "'";
		mysqli_query($con, $sql);
