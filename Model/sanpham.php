...
class sanpham
{
	...
	function lockSP($masp,$id)
	{
		$a = new DateBase;
		$con = $a->connnection();
		$sql = "update sanpham set TrangThai='".$id."' where MaSanPham='" . $masp . "'";
		...
		if($_SESSION['Quyen']=='1'){
			...
			echo "<script>window.location='thukhoindex.php';
				</script>";
		}
	}
	function changeSP($masp, $sanpham)
	{
		$a = new DateBase;
		$con = $a->connnection();
		$sql = "update sanpham set TenSanPham='" . $sanpham['TenSanPham'] . "',LoaiShow='" . $sanpham['LoaiShow'] . "',Gia='" . $sanpham['Gia'] . "',MoTa='" . $sanpham['MoTa'] . "',Anh='" . $sanpham['Anh'] . "' where MaSanPham='" . $masp . "'";
		mysqli_query($con, $sql);
		if($_SESSION['Quyen']=='1')
